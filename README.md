...
- Tools (CSV export, PDF spec sheets, 3D render, validation) live in `tools/` and are driven by
...
python -m meble compile-scene --set kitchen         # -> out/scene.json (then render with Blender)
```

See `.claude/skills/` for the guided workflows (`design-cabinet`, `generate-order-csv`,
`generate-panel-pdf`, `render-3d`, `validate-design`).
